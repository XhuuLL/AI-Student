        ...
        <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-md">
          <div className="mb-8 text-center">
            ...
            <p className="mt-2 text-sm text-foreground/70">
              Bergabunglah dan mulai perjalanan belajarmu.
            </p>
          </div>
          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-5">
            {/* Input Nama */}
            <div>
              <label className="mb-2 block text-sm font-medium text-foreground/90">Nama Lengkap</label>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  className="w-full rounded-xl border border-white/10 bg-white/5 py-3 pl-10 pr-10 text-sm outline-none transition-all placeholder:text-foreground/40 focus:border-emerald-500/50 focus:bg-transparent focus:ring-1 focus:ring-emerald-500/50"
                  value={password}
                ...
                {/* Tombol Show/Hide */}
                <button
                  type="button" // Wajib type="button" agar tidak memicu form submit saat diklik
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-foreground/50 transition-colors hover:text-foreground/80 focus:outline-none"
                >
                  {showPassword ? (
